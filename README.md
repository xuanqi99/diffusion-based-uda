...
CAN, GTA, ADDA, MCD, SymmNets, GVB-GD, ETD, SRDC, ACTIR, TCM, ICDA, iMSDA, UniOT, WDGRL, PPOT, CPH, SAMB-D, and TCRL baselines under `uda/`. These standard UDA methods are
...
Run an OfficeHome ICDA baseline:
...
The ICDA entry follows `Implicit Class-Conditioned Domain Alignment for
Unsupervised Domain Adaptation` (ICML 2020) with pseudo-label-guided
class-conditioned sample selection for domain-adversarial alignment.

Run an OfficeHome iMSDA baseline:
...
The iMSDA-style entry follows `Partial disentanglement for domain adaptation`
(ICML 2022) with a feature-level partially identifiable VAE, invariant/changing
latent partitioning, domain-specific invertible changing-part normalization,
source classification, and target entropy minimization.

Run an OfficeHome UniOT baseline:
...
The UniOT-style entry follows `Unified Optimal Transport Framework for
...
Run an OfficeHome PPOT baseline:
...
The PPOT entry follows `Probability-Polarized Optimal Transport for
...
Run an OfficeHome SAMB-D baseline:
...
The SAMB-D-style entry follows `Semantic-aware Message Broadcasting for
Efficient Unsupervised Domain Adaptation` with learned semantic group tokens,
message-broadcast feature refinement, domain-adversarial group-message
alignment, and target pseudo-label self-training.

Run an OfficeHome TCRL baseline:
...
The TCRL-style entry follows `Triplet Contrastive Representation Learning for
Unsupervised Vehicle Re-Identification` with global/part embeddings,
cluster-prototype contrastive learning, supervised contrastive learning,
batch-hard triplet regularization, and target pseudo-label adaptation.
