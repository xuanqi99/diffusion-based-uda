...
class GradientReverseFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output: torch.Tensor) -> tuple[torch.Tensor, None]:
        # DANN maximizes domain confusion by reversing gradients flowing into the feature extractor.
        return -ctx.coefficient * grad_output, None


def gradient_reverse(features: torch.Tensor, coefficient: float) -> torch.Tensor:
    return GradientReverseFunction.apply(features, coefficient)
...
class DANN(nn.Module):
    ...
    def forward(self, source_images: torch.Tensor, target_images: torch.Tensor, grl_lambda: float):
        ...
        # The discriminator sees both domains; the GRL makes features domain-invariant.
        domain_logits = self.domain_discriminator(gradient_reverse(domain_features, grl_lambda))
        return class_logits, domain_logits


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="DANN baseline for UDA datasets.")
...
def main() -> None:
    ...
    if bundle.target_train is None:
        raise ValueError("DANN requires a target training loader.")
    ...
    print(f"Method: DANN | model: {args.arch} | classes: {bundle.num_classes} | device: {device}")
...
def train_one_epoch(model, source_loader, target_loader, optimizer, scaler, device, epoch: int, args) -> dict[str, float]:
    ...
    for step, (source_images, source_labels) in enumerate(source_loader, start=1):
        ...
        with autocast_context(enabled=args.amp and device.type == "cuda", device_type=device.type):
            class_logits, domain_logits = model(source_images, target_images, grl_lambda)
            class_loss = F.cross_entropy(class_logits, source_labels)

            # Source is domain 0 and target is domain 1; target class labels are never used.
...
@torch.no_grad()
def evaluate(model: DANN, loader, device) -> dict[str, float]:
    model.eval()
    losses = AverageMeter()
...
def compute_grl_lambda(epoch: int, step: int, total_steps: int, args) -> float:
    if args.grl_schedule == "none":
        return args.grl_lambda
    progress = ((epoch - 1) * total_steps + step) / max(args.epochs * total_steps, 1)
    return args.grl_lambda * (2.0 / (1.0 + math.exp(-10.0 * progress)) - 1.0)
...
def make_output_dir(args) -> Path:
    if args.output_dir:
        ...
        output_dir = Path("runs") / f"dann_{args.dataset}_{args.source}_to_{args.target}_{stamp}"
